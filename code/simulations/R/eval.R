...
# For simulations with no factors
if(is.null(dat$F_mat)){
    dat$F_mat <- diag(ncol(dat$beta_hat))
    dat$L_mat_marg <- dat$beta_marg
    dat$theta_marg <- matrix(0, nrow = nrow(dat$beta_hat), ncol = ncol(dat$beta_hat))
}
...
if(snakemake@wildcards[["method"]]  %in%  c("ml", "guide", "factorgo")){
   df <- purrr::map_dfr(seq(length(fit)), function(i){
    ...
    df <- purrr::map_dfr(1:length(fit$n_factors), function(i){
                             dff <- eval(fit, dat, n_subset = fit$n_factors[[i]])
